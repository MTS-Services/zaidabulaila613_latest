...
export default function CategorySection() {
  ...
  const catAr = cat.map((el, index) => {
    return {
      name: el.name.ar,
      id: el.id,
      backgroundColor: '#CC9765',
      hoverColor: '#CC9765',
      textColor: '#383838',
    };
  });
  const catEn = cat.map((el, index) => {
    return {
      name: el.name.en,
      id: el.id,
      backgroundColor: '#CC9765',
  ...
  useEffect(() => {
    const updateCardsPerSlide = () => {
      ...
      let calculatedCardsPerSlide = Math.floor(
        availableWidth / (dimensions.cardWidth + dimensions.cardGap)
      );
      calculatedCardsPerSlide = Math.max(
        1,
        Math.min(calculatedCardsPerSlide, categories.length)
      );
    ...
    <section className='py-12 md:py-16'>
      <div className='container px-2 md:px-4'>
        <div className='flex items-center justify-between mb-8'>
          <h2 className='text-2xl font-bold tracking-tighter font-playfair'>
                    ...
                    >
                      ...
                      <div className='absolute inset-0 flex items-center justify-center'>
                        ...
                        <h3
                         ...
                         ${
                           hoveredIndex === index
                             ? 'opacity-0 rotate-90 transform scale-50'
                             : 'opacity-100 rotate-90 transform scale-100'
                         }
                       `}
                        >
                          {category.name}
                        ...
                        <h3
                         ...
                         text-sm md:text-2xl font-medium ${
                           category.textColor
                         } text-center px-2 md:px-4 absolute
                         transition-all duration-300 ease-in-out
                         ${
                           hoveredIndex === index
