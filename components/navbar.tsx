  ...
  return (
    ...
    >
      ...
      >
        ...
        {isDashboard ? (
          <div
            className='flex flex-col divide-y pl-2'
            onClick={() => setIsDrawerOpen(false)}
          >
            <CategoryItem
              href={'/dashboard'}
            ...
            <CategoryItem
              ...
              label={t('dashboard.sidebar.order')}
            />
            <CategoryItem
              href={'/dashboard/dress'}
              label={t('dashboard.sidebar.dress')}
            />
            <CategoryItem
            ...
            <CategoryItem
              href={'/dashboard/shop'}
              label={t('dashboard.sidebar.shop')}
            />
              ...
              <div className='grid grid-cols-2 w-full'>
                <button
                  className={`py-3 font-medium text-center ${
                    activeTab === 'menu'
                      ? 'bg-white border-b-2 border-gold'
                      : 'bg-slate-50'
                  }`}
              ...
              /> */}
                ...
                <div className='flex flex-col divide-y'>
                  {displayCategories.map((el) => {
                    return <CategoryItem href={el.href} label={el.label} />;
                  })}
                </div>
              )}
            </div>
...
function NavLink({ href, label }: { href: string; label: string }) {
  return (
    <Link
      href={href}
      className='text-slate-700 hover:text-gold font-medium transition-colors'
    >
      {label}
    </Link>
  );
...
function CategoryItem({
  ...
  return (
    <Link
      href={href}
      className='flex items-center gap-3 py-3 px-4 hover:bg-slate-50'
    >
      {icon && <span className='text-slate-500'>{icon}</span>}
      <span>{label}</span>
    </Link>
