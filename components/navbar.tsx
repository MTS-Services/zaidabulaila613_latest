...
export default function Navbar() {
  ...
  const displayProductTypes = (
    language === 'AR' ? arProductTypes : enProductTypes
  )?.map((el) => {
    return {
      label: el,
      ...
      >
        <div className='container px-4 lg:px-6 flex h-16 items-center justify-between'>
          ...
          <div className='flex items-center gap-8'>
            ...
            {/* Navigation Links */}
            <div
              className={`hidden lg:flex items-center space-x-4 transition-transform duration-300 ${
                searchExpanded ? 'lg:-translate-x-4' : ''
              }`}
            >
              <NavLinkWithDropdown
                label={t('navbar.shop')}
          ...
          <div className='flex items-center gap-4 w-[750px] justify-end'>
            ...
            {isAuthenticated && (
              ...
              >
                ...
                <div className='flex gap-2 items-center w-full'>
                  ...
                  <p className='text-gold-dark mb-0 font-semibold'>
            ...
            {/* Account Icon with Dropdown for logged-in users */}
            {isAuthenticated && user ? (
              ...
              >
                <button
                  className='text-slate-700 hover:text-slate-900 relative p-2 rounded-full hover:bg-slate-100 transition-colors flex items-center'
                  aria-haspopup='true'
                ...
                <div
                  className={`absolute right-0 top-full mt-2 w-56 bg-white rounded-md shadow-lg py-2 z-50 transition-opacity duration-200 border border-slate-100
                    ${
                      accountMenuOpen
                ...
                >
                  <button
                    className='flex items-center gap-2 w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 border-b border-slate-100'
                    onClick={() =>
                      (window.location.href = '/dashboard/profile')
                    }
                  >
                    <UserIcon className='h-4 w-4 text-gold' />
                    {user.user?.account?.firstName || 'Profile'}
                  </button>
                  <button
                    className='flex items-center gap-2 w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 border-b border-slate-100'
              ...
              // Not logged in: show normal dashboard icon
              <Link
                href='/dashboard'
            ...
            {/* Mobile: Account icon with dropdown on click */}
            {isAuthenticated && user && (
        ...
        <div className='hidden lg:block w-full border-t border-slate-100 bg-white'>
          <div className='container px-4 lg:px-6 flex items-center justify-between h-10'>
            ...
            <div className='text-sm text-slate-600'>
              <span className='text-gold font-medium'>{t('navbar.offer')}</span>
            </div>
...
// Add NavLinkWithDropdown component
