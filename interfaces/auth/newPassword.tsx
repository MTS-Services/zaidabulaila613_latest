...
import { FormSchemaSignUpType, resetPasswordValidator, signUpValidator } from '@/lib/validators/auth';
import { useRouter } from 'next/navigation';
import { useMutation, useQuery } from '@apollo/client';
import { CONFIRM_TOKEN_MUTATION, CREATE_USER_MUTATION } from '@/graphql/mutation';
...
const formFields: Field[] = [
    ...
    { type: "Input", name: 'password', label: 'New Password', inputType: 'password' },
    { type: "Input", name: 'confirmPassword', label: 'Confirm New Password', inputType: 'password' },
];
...
export default function ResetPassword({ token }: { token: string }) {
    ...
    const router = useRouter()
    const { login } = useAuth();
    const [confirmToken, { data, loading, error }] = useMutation(CONFIRM_TOKEN_MUTATION)
    const handleSubmit = async (values: any) => {
        ...
        try {
            ...
            // const result = response?.data?.forgetPassword;
            console.log(data, "Data")
            enqueueSnackbar({ message: "Password reset successfully.", variant: 'success', anchorOrigin: { horizontal: "center", vertical: "bottom" } })
            router.push('/login')
        } catch (e: any) {
