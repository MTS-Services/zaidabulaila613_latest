...
export default function VerifyOTP() {
  ...
  useEffect(() => {
    const savedIdentifier = sessionStorage.getItem('verificationIdentifier');
    const savedVerificationType = sessionStorage.getItem('verificationType');
    ...
    if (savedIdentifier && savedVerificationType) {
      ...
      enqueueSnackbar(
        'Could not find verification data. Please sign up again.',
        { variant: 'error' }
      );
      router.push('/signup');
    }
  ...
  const handleVerify = useCallback(
    async (event: React.FormEvent) => {
      ...
      setIsVerifying(true);
      try {
        const apiUrl = `${process.env.NEXT_PUBLIC_API_URL}/user/verify-signup-otp`;
  ...
  const handleResendOtp = useCallback(async () => {
    setIsResending(true);
    try {
      const apiUrl = `${process.env.NEXT_PUBLIC_API_URL}/user/resend-verification-otp`;
