...
const Users = () => {
  ...
  const { loading, error, data, refetch } = useQuery<UserProductsResponse>(
    GET_USER_PRODUCTS,
    {
      variables: {
        language: language,
  ...
  const products =
    data?.userProducts?.data?.map((product) => ({
      ...product,
    })) || [];
        ...
        <div>
          ...
          <button
            ...
            className={`p-2 rounded ${
              approve ? "bg-black text-white" : "bg-gray-200"
            } text-xs`}
          >
        ...
        <div className="flex gap-2">
          ...
          {bulkUpload ? (
            <Button
              className="mr-4"
              onClick={() => {
                router.push(`/dashboard/dress/import`);
              }}
            >
              {t("common.import")}
            </Button>
