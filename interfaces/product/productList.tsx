...
export default function ProductList({
  ...
  if (colors) {
    ...
    if (validColors) queryVariables.colors = validColors;
  }
  if (categoryIds) {
    const validcategoryIds = categoryIds.split(',');
    if (validcategoryIds) queryVariables.categoryIds = validcategoryIds;
  }
  if (vendorParam) {
    const validVendors = vendorParam.split(',');
    if (validVendors) queryVariables.userIds = validVendors;
  }
  if (sizeParam) {
    const validSizes = sizeParam.split(',');
    if (validSizes) queryVariables.sizes = validSizes;
  }
  ...
  const totalPages = data?.products?.total
    ? Math.ceil(data?.products?.total / 10)
    : 1;
        ...
        <div className='flex flex-col gap-6'>
          ...
          <div className='bg-white p-4 rounded-lg shadow-sm flex flex-wrap items-center justify-between gap-4'>
            ...
            <div className='text-sm text-slate-500'>
              {t('productpage.showing')} {count} {t('productpage.products')}
            </div>
          </div>
