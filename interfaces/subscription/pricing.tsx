...
const Pricing = () => {
  ...
  const currentSubscription =
    subscriptionData?.getMySubscription || user?.subscription;
  ...
  const handleUpgradeWithCancellation = async (
    newPriceId: string,
    planName: string
  ) => {
  ...
  const hasSubscriptionForPlanAndDuration = (
    planName: string,
  ...
  const hasSubscriptionForPlan = (planName: string) => {
    ...
    return Object.values(plan.prices).some(
      (price) => currentSubscription.stripePriceId === price.priceId
    );
  };

  // Helper function to get current subscription duration for a plan
  const getCurrentSubscriptionDuration = (
    planName: string
  ): PlanDuration | null => {
  ...
  const shouldShowDurationUpgrade = (
    planName: string,
                ...
                return (
                  ...
                  >
                    ...
                    <div className='px-6 pt-6 pb-8 bg-gray-50 sm:p-10 sm:pt-6'>
                      ...
                      <div className='mt-8'>
                        {hasSubscriptionForPlanAndDuration(
                          plan.name,
                          selectedDuration
                        ) && (
                          <div>
                            ...
                            <button
                              ...
                              onClick={() => {
                                const currentDuration =
                                  getCurrentSubscriptionDuration(plan.name);
                            ...
                            >
                              {isUpgrading ? (
                                <>
                                  ...
                                  'yearly' && selectedDuration === 'monthly' ? (
                                t('dashboard.subcription.switch_to_monthly')
                              ) : getCurrentSubscriptionDuration(plan.name) ===
                                  'monthly' && selectedDuration === 'yearly' ? (
                                t('dashboard.subcription.upgrade')
                              ) : (
                                ''
                              )}
